...
n1_values = [20, 50, 100, 150, 200]
n2_values = [50, 100, 200, 300, 400, 500]

# Default values
default_n1 = 50
default_n2 = 200

# Pre-generate all data combinations
all_data = {}
for n1 in n1_values:
    for n2 in n2_values:
        samples_n1 = np.random.binomial(n1, true_p, num_samples) / n1
        samples_n2 = np.random.binomial(n2, true_p, num_samples) / n2
...
# Add initial traces (n1=50, n2=200)
initial_data = all_data[(default_n1, default_n2)]
fig.add_trace(go.Histogram(
    x=initial_data['samples_n1'],
    name=f'n₁={default_n1} (SE={initial_data["se_n1"]:.4f})',
...
fig.add_trace(go.Histogram(
    x=initial_data['samples_n2'],
    name=f'n₂={default_n2} (SE={initial_data["se_n2"]:.4f})',
    marker_color='coral',
    opacity=0.6,
    nbinsx=35,
    histnorm='probability',
...
# Add true p line
fig.add_vline(x=true_p, line_dash="dash", line_color="red", 
              annotation_text=f"True p = {true_p}")

# Create 2 separate sliders vertically stacked on the right
sliders = [
    # Slider 1: n₁ (top right)
    dict(
        active=n1_values.index(default_n1),
        currentvalue={"prefix": "n₁: ", "font": {"size": 11}},
        pad={"t": 10, "b": 10, "l": 20, "r": 10},
        len=0.18,
        x=0.78,
        y=0.70,
        steps=[
            dict(
                method="restyle",
                args=[{
                    "x": [all_data[(n1, default_n2)]['samples_n1'],
                          all_data[(n1, default_n2)]['samples_n2']],
                    "name": [f'n₁={n1} (SE={all_data[(n1, default_n2)]["se_n1"]:.4f})',
                            f'n₂={default_n2} (SE={all_data[(n1, default_n2)]["se_n2"]:.4f})']
                }],
                label=str(n1)
            ) for n1 in n1_values
        ]
    ),
    # Slider 2: n₂ (bottom right)
    dict(
        active=n2_values.index(default_n2),
        currentvalue={"prefix": "n₂: ", "font": {"size": 11}},
        pad={"t": 10, "b": 10, "l": 20, "r": 10},
        len=0.18,
        x=0.78,
        ...
        steps=[
            dict(
                method="restyle",
                args=[{
                    "x": [all_data[(default_n1, n2)]['samples_n1'],
                          all_data[(default_n1, n2)]['samples_n2']],
                    "name": [f'n₁={default_n1} (SE={all_data[(default_n1, n2)]["se_n1"]:.4f})',
                            f'n₂={n2} (SE={all_data[(default_n1, n2)]["se_n2"]:.4f})']
                }],
                label=str(n2)
            ) for n2 in n2_values
        ]
...
fig.update_layout(
    title=dict(text="Sampling Distribution vs Sample Size", font=dict(size=14)),
    xaxis=dict(title="Sample Proportion", domain=[0, 0.75]),
    ...
    height=450,
    width=900,
    margin=dict(t=50, b=50, l=50, r=220),
    sliders=sliders
)
...
# A/B Test demonstration showing power and Type I error with interactive controls
np.random.seed(42)

# Parameter ranges for interactivity
n_per_arm_range = [50, 100, 200, 400, 800]
true_uplift_range = [0.0, 0.01, 0.02, 0.03, 0.05]
alpha_range = [0.01, 0.05, 0.10]
n_trials_range = [100, 250, 500, 1000]
...
# Pre-generate results for all combinations
all_traces = {}
for n_per_arm in n_per_arm_range:
    for true_uplift in true_uplift_range:
        for alpha in alpha_range:
            for n_trials in n_trials_range:
                # Run simulation
                samples = []
                for _ in range(n_trials):
                    successes_a = np.random.binomial(n_per_arm, baseline_p)
                    successes_b = np.random.binomial(n_per_arm, baseline_p + true_uplift)
                    ...
                    # Calculate p-value
                    pooled_p = (successes_a + successes_b) / (2 * n_per_arm)
                    se = np.sqrt(2 * pooled_p * (1 - pooled_p) / n_per_arm)
                    z_stat = np.abs(diff) / se if se > 0 else 0
                    p_value = 2 * (1 - stats.norm.cdf(z_stat))
                    
                    samples.append({'diff': diff, 'significant': p_value < alpha})
                
                sig_rate = np.mean([s['significant'] for s in samples])
                
                # Determine if this is null or alternative scenario
                scenario_type = "Null" if true_uplift == 0.0 else "Alt"
                metric_name = "Type I" if true_uplift == 0.0 else "Power"
                color = 'lightcoral' if true_uplift == 0.0 else 'steelblue'
                
                all_traces[(n_per_arm, true_uplift, alpha, n_trials)] = {
...
# Add initial traces (null and alternative)
null_key = (default_n, 0.0, default_alpha, default_trials)
alt_key = (default_n, default_uplift, default_alpha, default_trials)

for key in [null_key, alt_key]:
    trace = all_traces[key]
    fig.add_trace(go.Histogram(
        x=trace['diffs'],
        name=f"{trace['scenario_type']} ({trace['metric_name']} = {trace['sig_rate']:.1%})",
...
fig.add_vline(x=default_uplift, line_dash="dash", line_color="red",
              annotation_text=f"True effect = {default_uplift:.1%}", annotation_position="top right")

# Create 4 sliders vertically stacked on the right
sliders = [
    # Slider 1: Sample size per arm (top)
    dict(
        active=n_per_arm_range.index(default_n),
        currentvalue={"prefix": "n/arm: ", "font": {"size": 10}},
        pad={"t": 10, "b": 10, "l": 20, "r": 10},
        len=0.18,
        x=0.78,
        y=0.85,
        steps=[
            dict(
                ...
                args=[
                    {
                        "x": [all_traces[(n, 0.0, default_alpha, default_trials)]['diffs'],
                              all_traces[(n, default_uplift, default_alpha, default_trials)]['diffs']],
                        "name": [f"{all_traces[(n, 0.0, default_alpha, default_trials)]['scenario_type']} ({all_traces[(n, 0.0, default_alpha, default_trials)]['metric_name']} = {all_traces[(n, 0.0, default_alpha, default_trials)]['sig_rate']:.1%})",
                                f"{all_traces[(n, default_uplift, default_alpha, default_trials)]['scenario_type']} ({all_traces[(n, default_uplift, default_alpha, default_trials)]['metric_name']} = {all_traces[(n, default_uplift, default_alpha, default_trials)]['sig_rate']:.1%})"]
                    },
                    {
                        "title.text": f"A/B Testing: n={n}/arm, effect={default_uplift*100:.1f}pp, α={default_alpha:.2f}, trials={default_trials}<br>"
                                     f"<sub>Null Type I = {all_traces[(n, 0.0, default_alpha, default_trials)]['sig_rate']:.1%} | "
                                     f"Alt Power = {all_traces[(n, default_uplift, default_alpha, default_trials)]['sig_rate']:.1%}</sub>"
                    }
                ],
                label=str(n)
            ) for n in n_per_arm_range
        ]
    ),
    # Slider 2: True effect size
    dict(
        active=true_uplift_range.index(default_uplift),
        currentvalue={"prefix": "Effect: ", "font": {"size": 10}},
        pad={"t": 10, "b": 10, "l": 20, "r": 10},
        len=0.18,
        x=0.78,
        y=0.60,
        steps=[
            dict(
                ...
                args=[
                    {
                        "x": [all_traces[(default_n, 0.0, default_alpha, default_trials)]['diffs'],
                              all_traces[(default_n, uplift, default_alpha, default_trials)]['diffs']],
                        "name": [f"{all_traces[(default_n, 0.0, default_alpha, default_trials)]['scenario_type']} ({all_traces[(default_n, 0.0, default_alpha, default_trials)]['metric_name']} = {all_traces[(default_n, 0.0, default_alpha, default_trials)]['sig_rate']:.1%})",
                                f"{all_traces[(default_n, uplift, default_alpha, default_trials)]['scenario_type']} ({all_traces[(default_n, uplift, default_alpha, default_trials)]['metric_name']} = {all_traces[(default_n, uplift, default_alpha, default_trials)]['sig_rate']:.1%})"]
                    },
                    {
                        "title.text": f"A/B Testing: n={default_n}/arm, effect={uplift*100:.1f}pp, α={default_alpha:.2f}, trials={default_trials}<br>"
                                     f"<sub>Null Type I = {all_traces[(default_n, 0.0, default_alpha, default_trials)]['sig_rate']:.1%} | "
                                     f"Alt Power = {all_traces[(default_n, uplift, default_alpha, default_trials)]['sig_rate']:.1%}</sub>",
                        "shapes[1].x0": uplift,
                        "shapes[1].x1": uplift,
                        "shapes[1].label.text": f"True effect = {uplift:.1%}"
                    }
                ],
                label=f"{uplift*100:.0f}pp"
            ) for uplift in true_uplift_range if uplift > 0
        ]
    ),
    # Slider 3: Alpha level
    dict(
        active=alpha_range.index(default_alpha),
        currentvalue={"prefix": "α: ", "font": {"size": 10}},
        pad={"t": 10, "b": 10, "l": 20, "r": 10},
        len=0.18,
        x=0.78,
        y=0.35,
        steps=[
            dict(
                ...
                args=[
                    {
                        "x": [all_traces[(default_n, 0.0, a, default_trials)]['diffs'],
                              all_traces[(default_n, default_uplift, a, default_trials)]['diffs']],
                        "name": [f"{all_traces[(default_n, 0.0, a, default_trials)]['scenario_type']} ({all_traces[(default_n, 0.0, a, default_trials)]['metric_name']} = {all_traces[(default_n, 0.0, a, default_trials)]['sig_rate']:.1%})",
                                f"{all_traces[(default_n, default_uplift, a, default_trials)]['scenario_type']} ({all_traces[(default_n, default_uplift, a, default_trials)]['metric_name']} = {all_traces[(default_n, default_uplift, a, default_trials)]['sig_rate']:.1%})"]
                    },
                    {
                        "title.text": f"A/B Testing: n={default_n}/arm, effect={default_uplift*100:.1f}pp, α={a:.2f}, trials={default_trials}<br>"
                                     f"<sub>Null Type I = {all_traces[(default_n, 0.0, a, default_trials)]['sig_rate']:.1%} | "
                                     f"Alt Power = {all_traces[(default_n, default_uplift, a, default_trials)]['sig_rate']:.1%}</sub>"
                    }
                ],
                label=f"{a:.2f}"
    ...
    # Slider 4: Number of trials
    dict(
        active=n_trials_range.index(default_trials),
        currentvalue={"prefix": "Trials: ", "font": {"size": 10}},
        pad={"t": 10, "b": 10, "l": 20, "r": 10},
        len=0.18,
        x=0.78,
        y=0.10,
        steps=[
            dict(
                ...
                args=[
                    {
                        "x": [all_traces[(default_n, 0.0, default_alpha, trials)]['diffs'],
                              all_traces[(default_n, default_uplift, default_alpha, trials)]['diffs']],
                        "name": [f"{all_traces[(default_n, 0.0, default_alpha, trials)]['scenario_type']} ({all_traces[(default_n, 0.0, default_alpha, trials)]['metric_name']} = {all_traces[(default_n, 0.0, default_alpha, trials)]['sig_rate']:.1%})",
                                f"{all_traces[(default_n, default_uplift, default_alpha, trials)]['scenario_type']} ({all_traces[(default_n, default_uplift, default_alpha, trials)]['metric_name']} = {all_traces[(default_n, default_uplift, default_alpha, trials)]['sig_rate']:.1%})"]
                    },
                    {
                        "title.text": f"A/B Testing: n={default_n}/arm, effect={default_uplift*100:.1f}pp, α={default_alpha:.2f}, trials={trials}<br>"
                                     f"<sub>Null Type I = {all_traces[(default_n, 0.0, default_alpha, trials)]['sig_rate']:.1%} | "
                                     f"Alt Power = {all_traces[(default_n, default_uplift, default_alpha, trials)]['sig_rate']:.1%}</sub>"
                    }
                ],
                label=str(trials)
            ) for trials in n_trials_range
        ]
    )
]
...
fig.update_layout(
    title=dict(
        text=f"A/B Testing: n={default_n}/arm, effect={default_uplift*100:.1f}pp, α={default_alpha:.2f}, trials={default_trials}<br>"
             f"<sub>Null Type I = {all_traces[null_key]['sig_rate']:.1%} | Alt Power = {all_traces[alt_key]['sig_rate']:.1%}</sub>",
        font=dict(size=13)
    ),
    xaxis=dict(title="Observed Difference (p_B - p_A)", domain=[0, 0.75]),
    ...
    height=380,
    width=750,
    margin=dict(t=70, b=50, l=60, r=220),
    sliders=sliders
)
...
# P-Hacking demonstration: running NULL tests and seeing false positives
np.random.seed(123)

# Parameter ranges for interactivity
outcomes_range = [2, 3, 5, 8, 10]
subgroups_range = [2, 3, 4, 6, 8]
n_range = [50, 100, 200]
alpha_range = [0.01, 0.05, 0.10]
...
# Pre-generate results for all combinations
all_traces = {}
for outcomes in outcomes_range:
    for subgroups in subgroups_range:
        for n_per_test in n_range:
            for alpha in alpha_range:
                total_tests = outcomes * subgroups
                results = []
                
                # Run simulation
                for test_num in range(total_tests):
                    group1 = np.random.binomial(n_per_test, 0.5) / n_per_test
                    group2 = np.random.binomial(n_per_test, 0.5) / n_per_test
                    
                    pooled_p = (group1 + group2) / 2
                    se = np.sqrt(2 * pooled_p * (1 - pooled_p) / n_per_test)
                ...
                num_sig = sum(1 for r in results if r['significant'])
                colors = ['steelblue' if r['significant'] else 'lightgray' for r in results]
                
                all_traces[(outcomes, subgroups, n_per_test, alpha)] = {
                    'x': [r['test_num'] for r in results],
                    'y': [r['p_value'] for r in results],
                    'colors': colors,
                    'num_sig': num_sig,
...
# Add initial trace
initial_key = (default_outcomes, default_subgroups, default_n, default_alpha)
initial_trace = all_traces[initial_key]

fig.add_trace(go.Scatter(
    x=initial_trace['x'],
    y=initial_trace['y'],
    mode='markers',
    marker=dict(color=initial_trace['colors'], size=8),
    showlegend=False,
...
fig.add_hline(
    ...
    line_dash="dash", 
    line_color="red",
    annotation_text=f"α = {default_alpha:.2f}", 
    annotation_position="left"
)

# Create 4 sliders vertically stacked on the right
sliders = [
    # Slider 1: Number of outcomes (top)
    dict(
        active=outcomes_range.index(default_outcomes),
        currentvalue={"prefix": "Outcomes: ", "font": {"size": 10}},
        pad={"t": 10, "b": 10, "l": 20, "r": 10},
        len=0.18,
        x=0.78,
        y=0.85,
        steps=[
            dict(
                method="restyle",
                args=[{
                    "x": [all_traces[(out, default_subgroups, default_n, default_alpha)]['x']],
                    "y": [all_traces[(out, default_subgroups, default_n, default_alpha)]['y']],
                    "marker.color": [all_traces[(out, default_subgroups, default_n, default_alpha)]['colors']]
                }],
                label=str(out)
            ) for out in outcomes_range
        ]
    ),
    # Slider 2: Number of subgroups
    dict(
        active=subgroups_range.index(default_subgroups),
        currentvalue={"prefix": "Subgroups: ", "font": {"size": 10}},
        pad={"t": 10, "b": 10, "l": 20, "r": 10},
        len=0.18,
        x=0.78,
        y=0.60,
        steps=[
            dict(
                method="restyle",
                args=[{
                    "x": [all_traces[(default_outcomes, sub, default_n, default_alpha)]['x']],
                    "y": [all_traces[(default_outcomes, sub, default_n, default_alpha)]['y']],
                    "marker.color": [all_traces[(default_outcomes, sub, default_n, default_alpha)]['colors']]
                }],
                label=str(sub)
            ) for sub in subgroups_range
        ]
    ),
    # Slider 3: Sample size
    dict(
        active=n_range.index(default_n),
        currentvalue={"prefix": "n/test: ", "font": {"size": 10}},
        pad={"t": 10, "b": 10, "l": 20, "r": 10},
        len=0.18,
        x=0.78,
        y=0.35,
        steps=[
            dict(
                method="restyle",
                args=[{
                    "x": [all_traces[(default_outcomes, default_subgroups, n, default_alpha)]['x']],
                    "y": [all_traces[(default_outcomes, default_subgroups, n, default_alpha)]['y']],
                    "marker.color": [all_traces[(default_outcomes, default_subgroups, n, default_alpha)]['colors']]
                }],
                label=str(n)
            ) for n in n_range
        ]
    ),
    # Slider 4: Alpha level
    dict(
        active=alpha_range.index(default_alpha),
        currentvalue={"prefix": "α: ", "font": {"size": 10}},
        pad={"t": 10, "b": 10, "l": 20, "r": 10},
        ...
        steps=[
            dict(
                ...
                args=[
                    {
                        "x": [all_traces[(default_outcomes, default_subgroups, default_n, a)]['x']],
                        "y": [all_traces[(default_outcomes, default_subgroups, default_n, a)]['y']],
                        "marker.color": [all_traces[(default_outcomes, default_subgroups, default_n, a)]['colors']]
                    },
                    {
                        "shapes[0].y0": a,
                        "shapes[0].y1": a,
                        "shapes[0].label.text": f"α = {a:.2f}"
                    }
                ],
                label=f"{a:.2f}"
            ) for a in alpha_range
...
fig.update_layout(
    title=dict(
        text=f"P-Hacking Demo: {initial_trace['num_sig']}/{initial_trace['total_tests']} tests significant "
             f"({100*initial_trace['num_sig']/initial_trace['total_tests']:.0f}%)<br>"
             f"<sub>All NULL (no real effects) | Expected false positives: {initial_trace['alpha']*initial_trace['total_tests']:.1f}</sub>",
        font=dict(size=13)
    ),
    xaxis=dict(title="Test number (each dot = one hypothesis test)", domain=[0, 0.75]),
    yaxis_title="p-value",
    template="plotly_white",
    height=380,
    width=750,
    ...
    annotations=[
        dict(
            text="⚠️ Blue dots = false positives from random chance!",
...
def calculate_sample_size(baseline_p, mde, alpha, power):
    ...
    numerator = (z_alpha * np.sqrt(2 * p_bar * (1 - p_bar)) + 
                 z_beta * np.sqrt(baseline_p * (1 - baseline_p) + p2 * (1 - p2))) ** 2
    denominator = mde ** 2
    
    return int(np.ceil(numerator / denominator))

# Generate power curves for different MDE values
baseline_p = 0.42
alpha = 0.05
power = 0.80
mde_values = [0.01, 0.02, 0.03, 0.05, 0.08, 0.10]

fig = go.Figure()

# Create traces for each MDE value (3 traces per MDE: curve, power line, marker)
for i, mde in enumerate(mde_values):
    required_n = calculate_sample_size(baseline_p, mde, alpha, power)
    
    # Generate power curve
    max_n = int(max(3000, required_n * 1.5))
    n_values = np.arange(50, max_n, 50)
    p2 = baseline_p - mde
    p_bar = (baseline_p + p2) / 2
    z_alpha = stats.norm.ppf(1 - alpha / 2)
    ...
    for n in n_values:
        ...
        z_stat = mde / se
        power_values.append(1 - stats.norm.cdf(z_alpha - z_stat))
    
    # Default to showing MDE=0.03
    visible = True if i == 2 else False
    
    # Add power curve
    fig.add_trace(go.Scatter(
        x=n_values, y=power_values,
        mode='lines', line=dict(color='steelblue', width=3),
        name=f'MDE={mde*100:.0f}pp',
        visible=visible,
    ...
    # Add required n marker
    fig.add_trace(go.Scatter(
        x=[required_n], y=[power],
        mode='markers+text',
        ...
        visible=visible,
        showlegend=False,
        hovertemplate=f'Required: {required_n:,}/arm ({required_n*2:,} total)<extra></extra>'
    ))
...
for i, mde in enumerate(mde_values):
    ...
    # Create visibility array (3 traces per MDE value)
    visibility = [False] * (len(mde_values) * 3)
    visibility[i*3] = True      # Power curve
    visibility[i*3 + 1] = True  # Power line
    visibility[i*3 + 2] = True  # Marker
    
    step = dict(
        method="update",
        args=[{"visible": visibility}],
        label=f"MDE={mde*100:.0f}pp"
    )
    steps.append(step)

sliders = [dict(
    active=2,  # Default to MDE=0.03
    currentvalue={"prefix": "Minimum Detectable Effect: "},
    steps=steps
)]

fig.update_layout(
    title=dict(
        text=f"Power Analysis: Sample Size vs MDE<br>" +
             f"<sub>Baseline={baseline_p*100:.0f}% | Target power={power*100:.0f}% | α={alpha:.2f}</sub>",
        font=dict(size=14)
    ),
