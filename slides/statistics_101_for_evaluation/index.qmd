...
::: notes
Welcome! This 90-minute session builds statistical intuition for evaluation work. We're focusing on the mental models you need, not just formulas. Every concept connects directly to real civic tech evaluation scenarios.

Remind everyone: we're here to demystify stats, not intimidate. Questions are encouraged throughout.
:::
...
# Generate initial traces (n1=50, n2=200)
for n, color, name in [(50, 'steelblue', 'n₁=50'), (200, 'coral', 'n₂=200')]:
    samples = np.random.binomial(n, true_p, num_samples) / n
    theoretical_se = np.sqrt(true_p * (1 - true_p) / n)
    
    fig.add_trace(go.Histogram(
        x=samples,
        name=f'{name} (SE={theoretical_se:.4f})',
        marker_color=color,
        opacity=0.6,
        nbinsx=35,
...
# Generate hidden traces for all other combinations
for n1 in n1_values:
    for n2 in n2_values:
        if n1 == 50 and n2 == 200:  # Skip the initial visible combination
            continue
        
        for n, color, group_name in [(n1, 'steelblue', 'n1'), (n2, 'coral', 'n2')]:
            samples = np.random.binomial(n, true_p, num_samples) / n
            theoretical_se = np.sqrt(true_p * (1 - true_p) / n)
            name_display = f'n₁={n1}' if group_name == 'n1' else f'n₂={n2}'
            
            fig.add_trace(go.Histogram(
                x=samples,
                name=f'{name_display} (SE={theoretical_se:.4f})',
                marker_color=color,
                opacity=0.6,
                nbinsx=35,
                histnorm='probability',
...
for i, n1 in enumerate(n1_values):
    for j, n2 in enumerate(n2_values):
        ...
        # Calculate which traces to show
        if n1 == 50 and n2 == 200:
            step["args"][0]["visible"][0] = True
            step["args"][0]["visible"][1] = True
        else:
            idx = 2 + (i * len(n2_values) + j - (1 if i > 1 or (i == 1 and j > 1) else 0)) * 2
            step["args"][0]["visible"][idx] = True
            step["args"][0]["visible"][idx + 1] = True
        steps.append(step)

sliders = [dict(
    active=11,  # Index for n1=50, n2=200
    steps=steps,
    currentvalue={"prefix": "Sample sizes: "}
)]
...
# Add all traces (start with n=400, uplift=0.03, alpha=0.05, trials=500)
default_idx = None
for idx, trace in enumerate(all_traces):
    visible = (trace['n_per_arm'] == 400 and trace['true_uplift'] == 0.03 and 
               trace['alpha'] == 0.05 and trace['n_trials'] == 500)
    if visible and default_idx is None:
        default_idx = idx
...
# Group traces by scenario (null vs alt) for each parameter combination
# We need to show both null and alt for each setting
trace_groups = {}
for idx, trace in enumerate(all_traces):
    key = (trace['n_per_arm'], trace['true_uplift'], trace['alpha'], trace['n_trials'])
...
# Create slider steps
steps = []
for key in sorted(trace_groups.keys()):
    n_per_arm, true_uplift, alpha, n_trials = key
    
    # Skip if true_uplift is 0 (we'll show null separately)
    if true_uplift == 0.0:
        continue
    
    # Find the null trace for this configuration
    null_key = (n_per_arm, 0.0, alpha, n_trials)
    
    visibility = [False] * len(all_traces)
    
    # Show null trace
    if null_key in trace_groups:
        for idx in trace_groups[null_key]:
            visibility[idx] = True
    
    # Show alternative trace
    for idx in trace_groups[key]:
        visibility[idx] = True
    
    # Get statistics for title
    null_trace = all_traces[trace_groups[null_key][0]] if null_key in trace_groups else None
    alt_trace = all_traces[trace_groups[key][0]]
    ...
    step = dict(
        ...
        args=[
            ...
            {
                "title.text": (
                    f"A/B Testing Simulation: n={n_per_arm}/arm, effect={true_uplift*100:.1f}pp, α={alpha:.2f}, trials={n_trials}<br>"
                    f"<sub>Null Type I = {null_trace['sig_rate']:.1%} | Alt Power = {alt_trace['sig_rate']:.1%}</sub>"
                ),
                "annotations": [
                    dict(x=0, xref="x", y=0, yref="paper", 
                         text="Null = 0", showarrow=False, yshift=-40),
                    dict(x=true_uplift, xref="x", y=0, yref="paper",
                         text=f"True effect = {true_uplift:.1%}", showarrow=False, yshift=-40)
                ]
            }
        ],
        label=f"n={n_per_arm} eff={true_uplift*100:.0f}pp α={alpha:.2f} N={n_trials}"
    )
    steps.append(step)

# Create slider
sliders = [dict(
    active=next((i for i, s in enumerate(steps) if "n=400 eff=3pp α=0.05 N=500" in s['label']), 0),
    currentvalue={"prefix": "Settings: ", "font": {"size": 11}},
    pad={"t": 30},
    steps=steps
)]

# Add reference lines for default view
default_trace = all_traces[default_idx] if default_idx is not None else all_traces[0]
fig.add_vline(x=0, line_dash="dash", line_color="black")
fig.add_vline(x=default_trace['true_uplift'], line_dash="dash", line_color="red")

fig.update_layout(
    title=dict(
        text=f"A/B Testing Simulation: n={default_trace['n_per_arm']}/arm, effect={default_trace['true_uplift']*100:.1f}pp, α={default_trace['alpha']:.2f}, trials={default_trace['n_trials']}<br>"
             f"<sub>Explore how sample size, effect size, and alpha affect Type I error and Power</sub>",
        font=dict(size=13)
    ),
    xaxis_title="Observed Difference (p_B - p_A)",
...
**Key insight:** Use the slider to explore how sample size, effect size, and alpha affect Type I error (null scenario, coral) and statistical power (alternative scenario, blue). Larger samples increase power while maintaining Type I error control.
...
# Add all traces (start with outcomes=5, subgroups=4, n=100, alpha=0.05)
default_idx = None
for idx, trace in enumerate(all_traces):
    visible = (trace['outcomes'] == 5 and trace['subgroups'] == 4 and 
               trace['n'] == 100 and trace['alpha'] == 0.05)
    if visible:
        default_idx = idx
    ...
    fig.add_trace(go.Scatter(
        ...
        y=trace['y'],
        mode='markers',
        marker=dict(color=trace['colors'], size=8),
        showlegend=False,
        visible=visible,
...
steps = []
for idx, trace in enumerate(all_traces):
    visibility = [i == idx for i in range(len(all_traces))]
    ...
    step = dict(
        ...
        args=[
            ...
            {
                "title.text": (
                    f"P-Hacking Demo: {trace['num_sig']}/{trace['total_tests']} significant "
                    f"({100*trace['num_sig']/trace['total_tests']:.0f}%) | "
                    f"Expected: ~{trace['alpha']*trace['total_tests']:.1f}<br>"
                    f"<sub>All NULL (no effect) | {trace['outcomes']} outcomes × {trace['subgroups']} subgroups × n={trace['n']}/test</sub>"
                ),
                "annotations[0].text": (
                    f"⚠️ Blue dots = false positives! | Bonferroni: α = {trace['alpha']/trace['total_tests']:.4f}"
                )
            }
        ],
        label=f"Out={trace['outcomes']} Sub={trace['subgroups']} n={trace['n']} α={trace['alpha']}"
    )
    steps.append(step)

# Create slider
sliders = [dict(
    active=default_idx if default_idx is not None else 0,
    currentvalue={"prefix": "Settings: ", "font": {"size": 11}},
    pad={"t": 30},
    steps=steps
)]
...
default_trace = all_traces[default_idx] if default_idx is not None else all_traces[0]
fig.add_hline(
    y=default_trace['alpha'], 
    line_dash="dash", 
    line_color="red",
    annotation_text=f"α = {default_trace['alpha']:.2f}", 
    annotation_position="left"
)

fig.update_layout(
    title=dict(
        text=f"P-Hacking Demo: {default_trace['num_sig']}/{default_trace['total_tests']} significant "
             f"({100*default_trace['num_sig']/default_trace['total_tests']:.0f}%) | "
             f"Expected: ~{default_trace['alpha']*default_trace['total_tests']:.1f}<br>"
             f"<sub>All NULL (no effect) | {default_trace['outcomes']} outcomes × {default_trace['subgroups']} subgroups × n={default_trace['n']}/test</sub>",
        font=dict(size=13)
    ),
    ...
    annotations=[
        dict(
            text=f"⚠️ Blue dots = false positives! | Bonferroni: α = {default_trace['alpha']/default_trace['total_tests']:.4f}",
...
**Key insight:** Adjust the parameters to see how testing multiple hypotheses inflates false positives. With more tests, you're almost guaranteed to find "significant" results by pure chance!
