...
# A/B Test demonstration showing power and Type I error
np.random.seed(42)

# Two scenarios: null case (no effect) and alternative case (real effect)
fig = go.Figure()

# Scenario 1: Null hypothesis is TRUE (no real difference)
baseline_p = 0.12
true_uplift_null = 0.0
n_per_arm = 400
alpha = 0.05
n_trials = 500
...
for _ in range(n_trials):
    ...
    successes_b = np.random.binomial(n_per_arm, baseline_p + true_uplift_null)
    ...
    samples_null.append({'diff': diff, 'significant': p_value < alpha})

# Scenario 2: Alternative hypothesis is TRUE (real effect exists)
true_uplift_alt = 0.035
samples_alt = []
for _ in range(n_trials):
    successes_a = np.random.binomial(n_per_arm, baseline_p)
    successes_b = np.random.binomial(n_per_arm, baseline_p + true_uplift_alt)
    ...
    z_stat = np.abs(diff) / se if se > 0 else 0
    p_value = 2 * (1 - stats.norm.cdf(z_stat))
    samples_alt.append({'diff': diff, 'significant': p_value < alpha})

# Calculate statistics
null_sig_rate = np.mean([s['significant'] for s in samples_null])
alt_sig_rate = np.mean([s['significant'] for s in samples_alt])

# Create comparison plot
fig.add_trace(go.Histogram(
    x=[s['diff'] for s in samples_null],
    name=f'Null (Type I = {null_sig_rate:.1%})',
...
fig.add_trace(go.Histogram(
    x=[s['diff'] for s in samples_alt],
    name=f'Alt (Power = {alt_sig_rate:.1%})',
...
fig.add_vline(x=true_uplift_alt, line_dash="dash", line_color="red",
              annotation_text=f"True effect = {true_uplift_alt:.1%}")

fig.update_layout(
    title="A/B Testing Simulation: Type I Error vs Power",
...
**Key insight:** When null is true (coral), ~5% are falsely significant (Type I error). When effect exists (blue), power determines detection rate.
...
- **False Discovery Rate (FDR) control:** More powerful alternative
- **Report ALL tests,** not just significant ones

**Red flags to watch for:**

- "We found that X worked for [oddly specific subgroup]"
- No pre-registered analysis plan
- Only reporting significant results from many tests
...
- Bonferroni: very conservative, divide α by number of tests
- Benjamini-Hochberg (FDR): less conservative, controls expected proportion of false discoveries
- Both implemented in standard statistical software
...
**Slides & tools:** [Link to materials]

**Next session:** [Preview of what's next in the course]

> *Questions? Reach out anytime!*

::: notes
Close with warmth and openness.

Mention:
- Slides will be shared via [platform]
- Tools are open-source at [GitHub link or similar]
- Office hours or follow-up session if needed
- Connect on social media / professional networks

Encourage continued learning:
- Practice on your own data
- Share your evaluations with the community
- Ask questions in [Slack/forum/etc]

Final thought: "You now have the foundations to design and interpret evaluations rigorously. The best way to learn is by doing. Go evaluate something, make mistakes, and iterate. That's how we all improve."

Applause, thanks, exit!
:::
