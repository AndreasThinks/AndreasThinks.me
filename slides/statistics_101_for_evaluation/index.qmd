...
n_per_test = 100
alpha = 0.05
total_tests = num_outcomes * num_subgroups

results = []

for test_num in range(total_tests):
    # Generate two groups with NO real difference (both from same distribution)
...
fig.add_hline(y=alpha, line_dash="dash", line_color="red",
              annotation_text=f"α = {alpha:.2f}", annotation_position="left")

fig.update_layout(
    title=dict(
        text=f"P-Hacking Demo: {num_sig}/{total_tests} significant ({100*num_sig/total_tests:.0f}%) | Expected: ~{alpha*total_tests:.0f}<br>" +
             f"<sub>All tests are NULL (no real effect) | 5 outcomes × 4 subgroups × n=100/test</sub>",
        font=dict(size=13)
    ),
    ...
    annotations=[
        dict(
            text=f"⚠️ Blue dots are 'significant' by chance! | Bonferroni correction: α = {alpha/total_tests:.4f}",
...
**Key insight:** With 20 tests at α=0.05, we expect ~1 false positive even when nothing is real. Here we got {num_sig}. This is p-hacking in action!
